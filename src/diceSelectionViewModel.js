var diceSelectionViewModel = function() {
    ...
    self.sumDieRolls = function(){
        self.RollResults.removeAll();
        ...
        if(passSum > 0){
            for(var x = 0; x < passSum; x++){
                self.RollResults.push("symbol symbol-success");
            }
        } else if(passSum < 0){
            for(var x = 0; x > passSum; x--){
                self.RollResults.push("symbol symbol-failure");
            }
        }
        var sideEffectSum = rolls[DIE_ADVANTAGE_RESULT] - rolls[DIE_THREAT_RESULT];
        if(sideEffectSum > 0){
            for(var x = 0; x < sideEffectSum; x++){
                self.RollResults.push("symbol symbol-advantage");
            }
        } else if(sideEffectSum < 0){
            for(var x = 0; x > sideEffectSum; x--){
                self.RollResults.push("symbol symbol-threat");
            }
        }

        if(rolls[DIE_TRIUMPH_RESULT] > 0){
            for(var x = 0; x < rolls[DIE_TRIUMPH_RESULT]; x++){
                self.RollResults.push("symbol symbol-triumph");
            }
        }
        if(rolls[DIE_DESPAIR_RESULT]){
            for(var x = 0; x < rolls[DIE_DESPAIR_RESULT]; x++){
                self.RollResults.push("symbol symbol-despair");
            }
        }
    }
